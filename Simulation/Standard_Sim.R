...
ocp <- purrr::map(t1, function(t) {
  purrr::map(t, ~ stand_recalc(., design, ssrMethod = "ocp"))
})

rocp <- purrr::map(t1, function(t) {
  purrr::map(t, ~ stand_recalc(., design, ssrMethod = "rocp", beta_0 = beta_0))
})

pz <- purrr::map(t1, function(t) {
  purrr::map(t, ~ stand_recalc(., design, ssrMethod = "pz", beta_tilde = beta_tilde))
})

gsd <- purrr::map(t1, function(t) {
  purrr::map(t, ~ stand_recalc(., design, ssrMethod = "gsd"))
})
